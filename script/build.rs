...
fn main() {
    ...
    write_startup_snapshot(
        ...
        "residual_lazy_audio_sources.rs",
        "RESIDUAL_LAZY_AUDIO",
        vec![deferred_web_audio_extension()],
    );
}
...
fn write_startup_snapshot(
    ...
    let snapshot_path = out_dir.join(snapshot_name);
    ...
    let out_path = out_dir.join(residual_name);
    fs::write(
        &out_path,
        format!(
            "// Generated by build.rs (see there) — lazy_loaded_* sources not baked\n\
             ...
             pub(crate) static {constant_prefix}_JS_SOURCES: &[(&str, &str)] = &[\n{js_rows}];\n\
             pub(crate) static {constant_prefix}_ESM_SOURCES: &[(&str, &str)] = &[\n{esm_rows}];\n"
        ),
    )
