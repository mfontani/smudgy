...
audio-output-unavailable = Аудіовихід недоступний ({ $cause }). Термінал продовжує працювати; sinkId "none" Web Audio залишається доступним. Перезапустіть Smudgy, щоб повторити спробу фізичного аудіо.
audio-web-disabled = Web Audio вимкнено ({ $cause }). Термінал продовжує працювати. Перезапустіть Smudgy, щоб повторити запуск аудіо.
