...
layouts-saved-partial = Збережено розкладку «{ $name }» — не вдалося включити { $count } збережених карток-заповнювачів.
...
audio-output-unavailable = Фізичний аудіовихід недоступний ({ $cause }). Термінал продовжує працювати; типовий вихід Web Audio і sinkId "none" працюють без звуку. Збережені налаштування буде застосовано під час наступного фізичного запуску.
...
audio-state-unmuted = Не вимкнено
audio-session-label = Сеанс {$id} · Сервер {$server} · Профіль {$profile}
audio-ack-applied = застосовано
audio-ack-preference = налаштування збережено, але не застосовано
audio-ack-failed = відмова виходу; новий стан не застосовано
audio-ack-package-unconfirmed = політику збережено; активну область пакета не підтверджено
audio-control-row = { $label }: { $volume }% — { $state } — { $acknowledgement }
audio-action-mute = Вимкнути звук
audio-action-unmute = Увімкнути звук
audio-trusted-row = { $label }: керується сесією (довірений Main; без заяви про окреме керування пакетом)
audio-notice-target-closed = Аудіоціль закрито; нічого не застосовано й не збережено.
audio-notice-applied-saved = Налаштування аудіо застосовано й збережено.
audio-notice-applied-save-failed = Аудіо змінено для цього запуску, але не вдалося зберегти: { $error }
audio-notice-failed = Не вдалося змінити аудіо; нічого не збережено: { $error }
audio-notice-output-dead = Аудіовихід відмовив; нічого не застосовано й не збережено. Перезапустіть, щоб повторити спробу.
audio-notice-preference-saved = Налаштування збережено, але не застосовано цього запуску; перезапустіть, щоб повторити спробу аудіо.
...
audio-announcement-failed = Не вдалося змінити аудіо; нічого не збережено.
