...
manifest-allowed-hosts = Дозволені мережеві цілі
manifest-host-format-help = ім'я хоста, ім'я хоста:порт, * (будь-який інтернет-хост/порт), *:порт (будь-який інтернет-хост на цьому порту), unix:/шлях/до/сокета або vsock:CID:PORT.
manifest-local-transport-warning = Записи сокетів Unix і VSock можуть отримувати доступ до локальних системних служб і вважаються дозволами критичного ризику.
...
permission-connect-to-unix = з'єднуватися із сокетом Unix
permission-connect-to-vsock = з'єднуватися з кінцевою точкою VSock
